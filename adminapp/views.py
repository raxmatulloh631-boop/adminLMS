...
def save_user_action(request, message, count_key):
    actions = request.session.get('actions', [])
    actions.append(message)
    request.session['actions'] = actions

    count = request.session.get(count_key, 0)
    request.session[count_key] = count + 1
...
def faculty_create(request):
    ...
    if request.POST and form.is_valid():
        obj = form.save()
        save_user_action(request, f"You created faculty: {obj.name}", 'faculty_count')
...
def kafedra_create(request):
    ...
    if request.POST and form.is_valid():
        obj = form.save()
        save_user_action(request, f"You created kafedra: {obj.name}", 'kafedra_count')
...
def subject_create(request):
    ...
    if request.POST and form.is_valid():
        obj = form.save()
        save_user_action(request, f"You created subject: {obj.name}", 'subject_count')
...
def teacher_create(request):
    ...
    if request.POST and form.is_valid():
        obj = form.save()
        save_user_action(request, f"You added teacher: {obj.first_name}", 'teacher_count')
...
def group_create(request):
    ...
    if request.POST and form.is_valid():
        obj = form.save()
        save_user_action(request, f"You created group: {obj.name}", 'group_count')
...
def student_create(request):
    ...
    if request.POST and form.is_valid():
        obj = form.save()
        save_user_action(request, f"You created student: {obj.first_name}", 'student_count')
...
def profile(request):
    actions = request.session.get('actions', [])
    total = (
        request.session.get('faculty_count', 0) +
        request.session.get('teacher_count', 0) +
        request.session.get('student_count', 0) +
        request.session.get('group_count', 0) +
        request.session.get('kafedra_count', 0) +
        request.session.get('subject_count', 0)
    )
